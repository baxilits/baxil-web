    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <header className="border-b border-zinc-800">
        <div className="mx-auto max-w-6xl px-6 py-8">
          <h1 className="text-4xl font-bold">BAXIL STORE</h1>
          <p className="mt-2 text-zinc-400">Top Up Robux Cepat dan Aman</p>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-6xl gap-8 p-6 lg:grid-cols-2">
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-8">
          <h2 className="mb-6 text-2xl font-bold">Data Pembelian</h2>

          <label className="text-sm text-zinc-400">Username Roblox</label>
          <input
            type="text"
            value={username}
            onChange={(e) => {
          ...
          <button
            onClick={checkRoblox}
            className="mt-3 rounded-xl bg-white px-5 py-3 font-bold text-black hover:bg-zinc-200 transition"
          >
            {checking ? "Memeriksa..." : "Cek Username"}
          </button>
          ...
          {/* VERIFIKASI UMUR */}
          <div className="mt-6 flex items-start gap-3 rounded-xl border border-zinc-700 bg-zinc-900 p-5">
            <input
              type="checkbox"
              id="adultVerify"
              checked={isAdult}
              onChange={(e) => setIsAdult(e.target.checked)}
              className="mt-1 h-5 w-5 cursor-pointer accent-white"
            />
            <label
              htmlFor="adultVerify"
              className="cursor-pointer text-sm leading-relaxed text-zinc-200"
            >
              Saya memastikan akun Roblox yang digunakan sudah berusia{" "}
              <strong className="text-white">18+</strong> dan data yang saya
              berikan benar.
            </label>
          </div>
        </section>
        ...
        {/* SUMMARY */}
        <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-8">
          <h2 className="mb-6 text-2xl font-bold">Ringkasan Pembayaran</h2>

          {/* AVATAR */}
          <div className="mb-8 rounded-xl border border-zinc-800 bg-black p-5">
            <h3 className="mb-4 font-bold">Avatar Roblox</h3>
            ...
            {robloxUser?.found ? (
              <div className="text-center">
                ...
                />
                <p className="mt-3 font-bold">{robloxUser.displayName}</p>
                <p className="text-sm text-zinc-400">@{robloxUser.username}</p>
                <p className="mt-2 text-green-400">✓ Username ditemukan</p>
              </div>
            ) : robloxUser?.found === false ? (
              <p className="text-red-400">✕ Username Roblox tidak ditemukan</p>
            ) : (
              <span className="text-zinc-500">Belum dicek</span>
            )}
          </div>
          ...
          <div className="space-y-5">
            ...
            {/* STATUS UMUR */}
            <div className="flex justify-between">
              <span className="text-zinc-400">Verifikasi Umur</span>
              <span className={isAdult ? "text-green-400" : "text-red-400"}>
                {isAdult ? "✓ Akun 18+" : "Belum Verifikasi"}
              </span>
            </div>
          ...
          <button
            ...
            className={`mt-8 w-full rounded-xl py-4 font-bold transition ${isValid && robloxUser?.found && isAdult
