...
export default function AdminPage() {
        ...
        try {
            const res = await fetch("/api/admin/stock", {
                ...
                body: JSON.stringify({
                    ...
                    // PERBAIKAN: Gunakan fallback jika robloxUser.username bernilai undefined
                    username: robloxUser?.username || username.trim(),
                    robux: Number(robux),
                }),
            });
    ...
    return (
        <main className="min-h-screen bg-black text-white p-8">
            <h1 className="text-4xl font-bold">BAXIL ADMIN</h1>
            <p className="mt-2 text-zinc-400">Management Stok Robux</p>

            {/* TAMBAH SUPPLIER */}
            <div className="mt-10 max-w-xl rounded-2xl border border-zinc-800 bg-zinc-950 p-8">
                <h2 className="mb-6 text-2xl font-bold">Tambah Supplier</h2>
                ...
                {robloxUser?.found && (
                    <div className="mb-5 rounded-xl border border-green-700 bg-green-950 p-4 text-center">
                        {/* PERBAIKAN: Berikan gambar cadangan jika robloxUser.avatar kosong */}
                        <img
                            src={robloxUser?.avatar || "/file.svg"}
                            alt="avatar"
                            className="mx-auto h-24 w-24 rounded-xl object-cover"
                        />

                        <p className="mt-3 font-bold">
                            {robloxUser?.displayName || robloxUser?.username || username}
                        </p>

                        <p className="text-green-400">✓ Username valid</p>
                    </div>
                )}
            ...
            {/* DAFTAR SUPPLIER */}
            <div className="mt-12">
                <h2 className="mb-5 text-2xl font-bold">Daftar Supplier</h2>

                {stocks.length === 0 ? (
                    <p className="text-zinc-500">Belum ada stok</p>
                ) : (
                    <div className="space-y-4">
                        {stocks.map((item) => (
                            ...
                            >
                                <p className="text-lg font-bold">{item.accountName}</p>
                                <p className="text-zinc-400">@{item.username}</p>
            ...
            {/* ORDER MASUK */}
            <div className="mt-16">
                <h2 className="mb-6 text-3xl font-bold">Order Masuk</h2>
                        ...
                        <table className="w-full">
                            <thead className="bg-zinc-900">
                                <tr>
                                    ...
                                    <th className="p-4 text-left">Robux</th>
                            ...
                            <tbody>
                                {orders.map((order) => (
                                    <tr key={order.id} className="border-t border-zinc-800">
                                        <td className="p-4">@{order.customerUsername}</td>
                                        <td className="p-4">
